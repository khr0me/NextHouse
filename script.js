/**
 * Next House Control Center - Script Principale
 * Versione: 3.0.0
 */
...
const NextHouse = {
  ...
  init: function () {
    ...
    console.log("Inizializzazione Next House Control Center 2.0...");
    ...
    console.log("Next House Control Center inizializzato con successo!");
  ...
  cacheElements: function () {
    this.elements = {
      ...
      simulationSection: document.getElementById("simulation"),
      ...
      // Elementi simulazione
      lightQuantitySlider: document.getElementById("lightQuantity"),
      tempVariationSlider: document.getElementById("tempVariation"),
      humidityVariationSlider: document.getElementById("humidityVariation"),
      energyBaseSlider: document.getElementById("energyBase"),
      startSimBtn: document.getElementById("startSim"),
      stopSimBtn: document.getElementById("stopSim"),
      resetSimBtn: document.getElementById("resetSim"),
      insertCountDisplay: document.getElementById("insertCount"),
      weeklyAverageDisplay: document.getElementById("weeklyAverage"),
      nextAverageDisplay: document.getElementById("nextAverage"),
      averagesListDisplay: document.getElementById("averagesList"),
    };
  },
  ...
  // Setup simulatore dispositivi
  setupSimulation: function () {
    const self = this;

    // Event listeners per gli slider
    if (this.elements.lightQuantitySlider) {
      this.elements.lightQuantitySlider.addEventListener("input", function () {
    ...
    if (this.elements.tempVariationSlider) {
      this.elements.tempVariationSlider.addEventListener("input", function () {
        const valueElement = this.nextElementSibling;
        if (valueElement) {
          valueElement.textContent = this.value;
        }
    ...
    if (this.elements.humidityVariationSlider) {
      this.elements.humidityVariationSlider.addEventListener(
        "input",
        function () {
          const valueElement = this.nextElementSibling;
          if (valueElement) {
    ...
    if (this.elements.energyBaseSlider) {
      this.elements.energyBaseSlider.addEventListener("input", function () {
        const valueElement = this.nextElementSibling;
        if (valueElement) {
          valueElement.textContent = this.value;
        }
    ...
    if (this.elements.startSimBtn) {
      this.elements.startSimBtn.addEventListener("click", function () {
        self.startSimulation();
      });
    }

    if (this.elements.stopSimBtn) {
      this.elements.stopSimBtn.addEventListener("click", function () {
        self.stopSimulation();
      });
    }

    if (this.elements.resetSimBtn) {
      this.elements.resetSimBtn.addEventListener("click", function () {
  ...
  startSimulation: function () {
    if (this.state.simulation.isRunning) return;

    this.state.simulation.isRunning = true;
    this.elements.startSimBtn.disabled = true;
    this.elements.stopSimBtn.disabled = false;

    // Avvia il loop di simulazione
    this.state.simulation.interval = setInterval(() => {
      this.generateSimulationData();
    }, 2000); // Ogni 2 secondi

    this.showNotification("Simulazione", "Simulazione avviata", "success");
  },
  ...
  stopSimulation: function () {
    ...
    this.state.simulation.isRunning = false;
    this.elements.startSimBtn.disabled = false;
    this.elements.stopSimBtn.disabled = true;

    if (this.state.simulation.interval) {
      clearInterval(this.state.simulation.interval);
      this.state.simulation.interval = null;
    }

    this.showNotification("Simulazione", "Simulazione fermata", "info");
  },
  ...
  resetSimulation: function () {
    ...
    this.showNotification("Simulazione", "Simulazione resettata", "info");
  },

  // Genera dati di simulazione
  generateSimulationData: function () {
    // Ottieni i valori dai controlli
    const lightQuantity = parseInt(this.elements.lightQuantitySlider.value);
    const tempVariation = parseFloat(this.elements.tempVariationSlider.value);
    const humidityVariation = parseInt(
      this.elements.humidityVariationSlider.value
    );
    const energyBase = parseFloat(this.elements.energyBaseSlider.value);

    // Genera valori casuali basati sui parametri
    const temperature = 22 + (Math.random() - 0.5) * 2 * tempVariation;
    const humidity = 45 + (Math.random() - 0.5) * 2 * humidityVariation;
    const energyConsumption = energyBase + (Math.random() - 0.5) * 0.5;

    // Calcolata basata sul numero di luci
    const activeDevices = lightQuantity + Math.floor(Math.random() * 4);

    // Crea il record di dati
    const dataPoint = {
      timestamp: new Date(),
      temperature: Math.round(temperature * 10) / 10,
      humidity: Math.round(humidity),
      energyConsumption: Math.round(energyConsumption * 100) / 100,
      activeDevices: Math.min(activeDevices, 20),
      lightQuantity: lightQuantity,
    };

    // Aggiungi ai dati di simulazione
    this.state.simulation.data.push(dataPoint);
    this.state.simulation.insertCount++;

    // Aggiorna i valori sul dashboard
    this.elements.temperatureDisplay.textContent = `${dataPoint.temperature}°C`;
    this.elements.humidityDisplay.textContent = `${dataPoint.humidity}%`;
    this.elements.energyConsumptionDisplay.textContent = `${dataPoint.energyConsumption} kWh`;
    this.elements.activeDevicesDisplay.textContent = `${dataPoint.activeDevices}/20`;

    // Verifica se è il momento di calcolare la media settimanale
    if (this.state.simulation.insertCount % 7 === 0) {
  ...
  calculateWeeklyAverage: function () {
    if (this.state.simulation.data.length < 7) return;

    // Prendi gli ultimi 7 record
    const lastWeekData = this.state.simulation.data.slice(-7);

    // Calcola le medie
    const averages = {
      week: this.state.simulation.weekCounter,
      temperature: lastWeekData.reduce((sum, d) => sum + d.temperature, 0) / 7,
      humidity: lastWeekData.reduce((sum, d) => sum + d.humidity, 0) / 7,
      energyConsumption:
        lastWeekData.reduce((sum, d) => sum + d.energyConsumption, 0) / 7,
      activeDevices:
        lastWeekData.reduce((sum, d) => sum + d.activeDevices, 0) / 7,
    };

    // Arrotonda i valori
    averages.temperature = Math.round(averages.temperature * 10) / 10;
    averages.humidity = Math.round(averages.humidity);
    averages.energyConsumption =
      Math.round(averages.energyConsumption * 100) / 100;
    averages.activeDevices = Math.round(averages.activeDevices);

    // Aggiungi alla lista delle medie
    this.state.simulation.weeklyAverages.push(averages);
    this.state.simulation.currentAverage = averages;
    this.state.simulation.weekCounter++;
    this.state.simulation.nextAverageCount = 7;

    // Mostra notifica
    this.showNotification(
      "Media Settimanale",
      `Settimana ${averages.week} completata. Media consumo: ${averages.energyConsumption} kWh`,
      "success"
    );
  },

  // Aggiorna visualizzazione simulazione
  updateSimulationDisplay: function () {
    // Aggiorna contatori
    this.elements.insertCountDisplay.textContent =
      this.state.simulation.insertCount;
    this.elements.nextAverageDisplay.textContent =
      7 - (this.state.simulation.insertCount % 7);

    // Aggiorna media corrente
    if (this.state.simulation.currentAverage) {
      this.elements.weeklyAverageDisplay.textContent = `${this.state.simulation.currentAverage.energyConsumption} kWh`;
    } else {
      this.elements.weeklyAverageDisplay.textContent = "-";
    }

    // Aggiorna lista medie settimanali
    if (this.state.simulation.weeklyAverages.length > 0) {
      this.elements.averagesListDisplay.innerHTML =
        this.state.simulation.weeklyAverages
          .map(
            (avg) => `
         <div class="average-item">
           <strong>Settimana ${avg.week}</strong>
           <span>Temp: ${avg.temperature}°C | Umidità: ${avg.humidity}% | Energia: ${avg.energyConsumption} kWh</span>
         </div>
       `
          )
          .join("");
