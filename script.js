...
const SmartHome = {
 ...
 setupSimulation: function () {
   ...
   if (this.elements.humidityVariationSlider) {
     this.elements.humidityVariationSlider.addEventListener("input", function () {
       const valueElement = this.nextElementSibling;
       if (valueElement) {
         valueElement.textContent = this.value;
       }
 ...
 generateSimulationData: function () {
   ...
   const humidityVariation = parseInt(this.elements.humidityVariationSlider.value);
   const energyBase = parseFloat(this.elements.energyBaseSlider.value);

   // Genera valori casuali basati sui parametri
   const temperature = 
     22 + (Math.random() - 0.5) * 2 * tempVariation;
   const humidity = 
     45 + (Math.random() - 0.5) * 2 * humidityVariation;
   const energyConsumption = 
     energyBase + (Math.random() - 0.5) * 0.5;
 ...
 calculateWeeklyAverage: function () {
   ...
   const averages = {
     ...
     energyConsumption: lastWeekData.reduce((sum, d) => sum + d.energyConsumption, 0) / 7,
     activeDevices: lastWeekData.reduce((sum, d) => sum + d.activeDevices, 0) / 7,
   };

   // Arrotonda i valori
   averages.temperature = Math.round(averages.temperature * 10) / 10;
   averages.humidity = Math.round(averages.humidity);
   averages.energyConsumption = Math.round(averages.energyConsumption * 100) / 100;
 ...
 updateSimulationDisplay: function () {
   // Aggiorna contatori
   this.elements.insertCountDisplay.textContent = this.state.simulation.insertCount;
   ...
   if (this.state.simulation.currentAverage) {
     this.elements.weeklyAverageDisplay.textContent = 
       `${this.state.simulation.currentAverage.energyConsumption} kWh`;
   } else {
     this.elements.weeklyAverageDisplay.textContent = "-";
   }
   ...
   if (this.state.simulation.weeklyAverages.length > 0) {
     this.elements.averagesListDisplay.innerHTML = this.state.simulation.weeklyAverages
       .map(avg => `
     ...
     this.elements.averagesListDisplay.innerHTML = "<p>Nessuna media calcolata</p>";
   }
 },
